...
- 修复折叠侧栏切换 Agent 项目时，当前会话与活动标签页没有随项目切换的问题。
...
- 恢复 Proma 原生外壳与交互；Linguist 复用通用 Agent 的顶栏、消息、输入框和右侧工作区，Chat 保留原生 Chat 界面。保留项目绑定、CAT 与独立产品身份。
...
- 重组本地化公共原则、四岗位 Prompt 与专业 Skills，增加按需读取的游戏本地化方法；区分双语审校与目标语校对，保留准确自然的既有表达。
- 项目摘要读取可选、版本化的派生简报，区分完整、部分、过期要求；完整客户规范与普通参考按不同责任读取。
- CAT 上下文使用同响应共享引用，先去重再分页，保留完整源译、关联与原生结构；明确当前未附资料和历史 Stage 覆盖的不同含义。
- 文档元数据支持显式独立续页，避免重复返回正文；默认写回返回每项实际提交 revision，失败项不产生成功回执。
- 新增受管工作区文件审校工作副本工具，复用现有格式适配器，组合完整 JSON 工作稿、差异与明确裁定，不修改原文件、CAT 阶段或外部工作状态。
- 现有 LA 面板展示当前批次阶段决定、建议生成依据及本次探测的 Skill 文件版本/hash；原生 Agent、Chat、会话和浏览器继续复用 Proma。
- ICU 允许合法的目标语复数类别变化，同时保留变量、分支业务含义和锁/CAS保护；显式项目语法扩展沿现有 Tag Profile 校验。
- Phrase 默认流程合并重复验证，区分保存、QA、客户 TM 与工作状态。较新的项目定制 Skill 保留，需要在新运行边界合并更新，不会在运行中整体覆盖。

### Validation and limits

- 新增非客户语言案例、未见小样与输入隔离检查，工程、真实模型、打包、生产证据分别记录于 `docs/release/VALIDATION_0_18_0.md`。
- Proma / Pi 版本、压缩阈值、摘要预算、模型和 reasoning effort 策略保持不变；本次不宣称彻底解决单次压缩长等待，不承诺未经实测的提速倍数。
- 客户批次、真实 Phrase 与游戏内 LQA 不作为开发试验；日用安装由用户通过在线更新完成。

## [0.17.75] - 2026-09-16

### Changed

- 浏览器支持显式文字与真实组合键，并可在单个标签页内串行完成有界的检查、编辑、等待和读回；停止或异常保留实际执行前缀。
- Linguist 与 Agent 共用原生侧栏项目头、会话树及工作区组件入口；关闭原生组件返回当前 CAT。
- Agent / Linguist 会话顶栏恢复上游高度、标题字号与工作区展开入口；项目徽标作为静态身份标识，保留岗位菜单。
- 定时任务可保存项目、岗位与明确批次/句段范围，在来源会话删除、重启和会话复用后保持正确绑定。
- 更新 Phrase 回填 Skill：复用批准稿、减少重复观察，分别记录保存、QA/TM 与句段确认；保持 Accepted 工作状态。

### Fixed

- 拒绝将旧式组合键名称意外写进译文；在实际输入节点派发前检查焦点和可选条件。
- 超过 100 段的截断选择不能被静默保存为完整任务范围；切换工作区会清除旧项目绑定。
- 定时任务运行状态明确表示执行结束，避免误报业务已经完成。

### Validation limits

- 本地 Chromium 与自动回归不等于真实 Phrase 校准；本轮未取得获准测试工作，不附带未校准的站点写入脚本或真实平台提速结论。
...
- Linguist 使用原生 Agent 主会话与右侧 CAT 工作区；批次源文件和 Context 预览在当前会话右侧打开。
- 项目设置区分批次与项目共用语言资产；辅助面板统一包含 TM、术语、QA、上下文、修改建议与准备交付。

### Fixed

- 修改建议默认显示当前批次待处理项，分页总数按批次统计；已接受历史不再因正常 revision 增加误报版本冲突。
- QA 列表、计数与下一项跟随当前批次；切批次时准备交付同步选择，并隔离迟到查询。
- 切换 CAT、预览与会话保留进程内草稿和撤销历史；恢复旧项目入口为原生会话，保留现有面板尺寸偏好。
- 批次导航采用右侧边线，避免向底部状态栏投射阴影。

## [0.17.72] - 2026-09-08

### Changed

- Proma 基线升级至 v0.19.37，合入 GitHub Copilot 订阅登录、计划审批预览、跨工作区定时任务以及预览、Skills、Vault 与更新缓存修复。
- 优化 CAT 邻文查询，降低大批次操作对主进程的阻塞。

### Fixed

- 当前应用进程内保留 CAT 编辑草稿，修复虚拟滚动、保存与项目切换竞态；保存期间冻结编辑，失败时保留草稿及其原始 revision。
- 改善窄窗与最小窗口的片段编辑、滚动访问及语言资产 Dock 高度偏好恢复；传统滚动条下保持表头与文本列对齐。
- 凭据加解密失败与损坏配置读取显式报错，配置写入使用独占临时文件及原子替换。
- 修复单语 JSON 译文导出校验及并发交付的版本证据一致性。
...
- 当前动态事实集中维护，README、工程规则、交接和状态入口不再复制版本或测试总数；纠正岗位身份限制、个人发布定位及原有 CAT 工具数漏记。

- 启动页、加载页和欢迎页共用 Linguist Agent 身份合同；首次引导介绍 Agent / Chat / Linguist，并更新隔离应用截图、岗位 FAQ 与产品可见文案。
