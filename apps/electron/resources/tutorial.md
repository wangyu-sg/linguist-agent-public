...
在刚刚的语言里，我们说明确地自然语言描述，这非常重要。人类跟人类表达的时候，很多时候我们是借助了环境作为上下文，省略了很多明确的表达，但因为人在环境和语境里，是可以理解的。比如你可以跟身边的同事说：帮我把那个那个下周的报表做一下给小李。这在人的理解里是有意义的，但是这句话不能直接输入给 Agent，这是个非常糟糕的语义。我们需要更明确地指示给 Agent，你需要的是尽可能完整的自然语言描述给 Agent 完整的上下文，这句话转换成一个好的 Agent 提示词可能是：在当前项目根目录下有个 @xx.excel 文档，这个文档是我们的本月销售报表，其中核心产品是我们的 a、b、c，我们的成本在另外一个 @xx.execl 文档，成本项和销售项的对照关系在 sheet2，我希望你先访问一下文档，然后帮我整理我们的销售毛利率到文档 @xx.excel 的 sheet1。关于具体的格式我们有一个说明，你可以先参照整理，如果数据有缺漏的部分可以明确指出，我来补充；有任何不确定的部分都停下来跟我确认；完成任务后给我一份你的分析方法的总结。
...
Proma 现在将文件分为三个不同层级。理解它们的边界，能让 Agent 的上下文更干净、输出更快也更稳定。

**会话文件**只服务当前一次任务，适合临时输入、计划、阶段性设计和本次任务的产物。直接把文件拖到输入框时，默认会加入当前会话。只和这次任务有关的资料，应该优先放在这里。

**项目根目录**是一个项目长期工作的主文件夹。新建空白项目时，Proma 会提供托管的项目目录；选择“从本地文件夹创建项目”时，所选的原始文件夹就是项目根目录，项目内所有会话都可以直接读写它。适合把代码库、长期参考资料、跨会话复用的产物和最终交付物放在这里。请注意：本地项目根目录中的修改会直接写入你的原始文件，它不是临时目录。

**附加目录和文件**是额外授权给当前会话或整个项目的既有资料，适合参考库、素材库、Obsidian 仓库等不想复制进项目根目录的内容。附加目录不是项目根目录，也不会自动成为 Agent 的默认工作目录。

例如开发 Proma 时，可以直接选择 Proma 代码库“从本地文件夹创建项目”；之后在这个项目下为每个功能新建会话，所有会话都会共享这份项目根目录。与当前功能无关的临时设计或输入，则继续放在对应会话文件中。

当然，不必第一次就做到完美。意识到某份资料下次还会用时，再把它整理进项目根目录即可；持续迭代上下文组织，本身就是最佳实践。

### 文件设计 —— 关于附加文件

附加文件和文件夹适合连接你已有的文件体系或其他软件的资料。文件本身可能很大、很多，或已有稳定位置时，以附加方式授权访问可以避免重复占用磁盘空间。

例如，你可以把 Obsidian 的原始目录作为附加目录，让 Proma 读取或编辑其中的内容；如果它是当前项目长期工作的核心目录，则更适合通过“从本地文件夹创建项目”把它设为项目根目录。

### 文件设计进阶 —— 主动设计更符合语义的文件

新会话直接使用会话工作台根目录存放本次任务的 Todo、Plan、临时笔记和 handoff；项目根目录下的 `.context/` 则用于跨会话共享的 Note、设计、进度和项目级知识。历史会话可能仍保留会话级 `.context/`，Proma 会按旧路径兼容。空白项目的项目根目录仍是 Proma 托管的 `workspace-files/`；本地项目则是你选择的原始文件夹。

如果任务很长、一次会话难以完成，可以让 Agent 在项目根目录的 `.context/` 中维护设计、进度追踪和踩坑记录。之后开启新会话时，告诉 Agent 继续阅读这份项目级 Context；不确定的部分还可以用 `&` 引用相关会话。这样就能让大型任务在多个会话中持续迭代。
