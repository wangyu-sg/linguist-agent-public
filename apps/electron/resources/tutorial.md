...
会话工作台和项目根目录下各有一层 `.context/`，但用途不同：会话级 `.context/` 用于本次任务的 Todo、Plan、临时笔记等；项目根目录下的 `.context/` 用于跨会话共享的 Note、设计、进度和项目级知识。空白项目的项目根目录仍是 Proma 托管的 `workspace-files/`；本地项目则是你选择的原始文件夹。
