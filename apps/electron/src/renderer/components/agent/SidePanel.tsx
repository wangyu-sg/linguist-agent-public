...
export function SidePanel({ sessionId, sessionPath, activeTab, onTabChange, width = 460 }: SidePanelProps): React.ReactElement {
  ...
  const effectiveActiveTab: AgentSidePanelTab = activeTab === 'chat' && !sideChatConversationId
    ...
    : activeTab === 'linguist' && hostWorkspace.tab === null
      ? 'files'
    : (activeExplorationSessionId !== null && !activeExplorationBranch) || (activeTab === 'delegation' && !selectedDelegationSession) || (activeTerminalId !== null && !terminalTabs.some((terminal) => terminal.terminalId === activeTerminalId))
      ? 'files'
  ...
  const handleCloseDelegationTab = React.useCallback(() => {
    ...
    if (activeTab === 'delegation') {
      returnToPreviousTabAfterClose('delegation')
    }
  ...
  ], [activeBrowserTabId, browserState, hostWorkspace.tab, previewFiles, selectedDelegationSession, selectedDelegationStatus, sessions, showBrowserActivity, sideChatConversationId, sideTemporaryAgents, terminalTabs, workspaceComponentTabs])
