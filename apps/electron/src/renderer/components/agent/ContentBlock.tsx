      ...
      >
        ...
        >
          ...
          {!isCompleted && isStreaming ? (
            <>
              <Loader2 className="size-3.5 shrink-0 animate-spin text-primary/60 motion-reduce:hidden" />
  ...
  return (
    ...
    >
      <button
        type="button"
        className={cn(
          'group inline-flex max-w-full min-w-0 items-center gap-2 rounded-md py-1 text-left',
          'transition-[background-color,opacity] hover:bg-muted/40 hover:opacity-90',
        )}
        onClick={() => setExpanded(!expanded)}
      >
        {!isCompleted && isStreaming ? (
