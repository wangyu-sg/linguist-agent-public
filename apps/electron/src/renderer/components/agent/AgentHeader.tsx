...
export function AgentHeader({ sessionId }: AgentHeaderProps): React.ReactElement | null {
  ...
  const [isRightPanelOpen, setRightPanelOpen] = useAtom(agentSidePanelOpenAtomFamily(sessionId))
      ...
      {editing ? (
        <div className="flex items-center gap-1.5 flex-1 min-w-0 titlebar-no-drag">
          <input
            ...
            onBlur={saveTitle}
            aria-label="会话标题"
            className="min-w-0 flex-1 border-b border-primary/50 bg-transparent px-0 py-0.5 text-[15px] font-normal outline-none"
            maxLength={100}
          />
          <button
            type="button"
