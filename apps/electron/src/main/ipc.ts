...
import { startTodoAgentWithRollback } from './lib/todo-agent-start'
import {
...
import { registerLinguistIpc, stopAllLinguistIntegrityScrubs } from './lib/linguist/register-ipc'
...
export { stopAllLinguistIntegrityScrubs }
...
export function resolveAppIconPath(_variantId: string): string | null {
  return join(getBundledResourcesDir(), 'icon.png')
}
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_, title?: string, channelId?: string, workspaceId?: string, modelId?: string, isDraft?: boolean): Promise<AgentSessionMeta> => {
      ...
      const session = createAgentSession(title, channelId, workspaceId, modelId, undefined, undefined, undefined, isDraft)
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      ...
      // Tombstone first so a headless run still in async preflight cannot start
      // or recreate its transcript after this handler removes session files.
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      ...
      for (const sessionId of affectedSessionIds) {
        // Use the same tombstone-and-drain boundary as single-session deletion.
  ...
  // 两者存储介质不同，顺序由 startTodoAgentWithRollback 保证可恢复：
  // 会话创建失败不改 Todo；Todo CAS 失败则删除刚建会话及其工作目录。
  ipcMain.handle(PLANNING_IPC_CHANNELS.START_TODO_AGENT, (_, input: StartTodoAgentInput): StartTodoAgentResult => {
    ...
    const { todo, session } = startTodoAgentWithRollback(existing, input, {
