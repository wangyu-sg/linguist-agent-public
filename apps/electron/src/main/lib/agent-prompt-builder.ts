...
- 先调用 \`BrowserObserve\`，再使用最新快照中的 ref 调用 \`BrowserClick\` 或 \`BrowserFill\`；快照过大或找不到目标时用 \`BrowserFind\` 按 role/name 返回少量新 ref。每次 Observe/Find、页面导航或重渲染都会作废该 tab 的旧 ref；时间流逝本身不会失效，但应在下一次 Observe/Find 前使用。已知点击后的预期状态时优先 \`BrowserAct\`（点击并等待，或最多 64 步、30 秒的串行操作）；其他等待使用 \`BrowserWaitFor\` 的 URL、文本或 selector 条件，不要用 JavaScript 自行轮询。 \`BrowserPress\` 用 action 明确区分 text 与 key/modifiers，target 指向实际输入节点，默认 verify 保留已有选区；guard 在写入前核对身份和内容。整段替换可用 \`BrowserFill\`，或带 guard 的 BrowserAct fill 步骤。不要将 Meta+A 等快捷键名称当作旧 key 字符串。
