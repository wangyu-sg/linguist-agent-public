 ...
 * - undoImportAsset：Proposal/QA/评审件/导出/人工编辑任一引用即拒绝
...
function makeCriticArtifact(segmentId: string) {
...
function assertUndoBlocked(
  err: unknown,
  expected: Partial<Record<'proposals' | 'qaFindings' | 'criticArtifacts' | 'exports' | 'editedSegments' | 'jobs', number>>,
...
test('undo blocked by QA findings and by critic artifacts (each category counted)', async () => {
  const service = makeService()
  try {
    const project = service.createProject(INPUT)
    ...
    bDb.criticArtifacts.insert(makeCriticArtifact(bSegments[0]!.id))
    assert.throws(
      () => service.undoImportAsset(project.id, bResult.assetId),
      (err: unknown) => {
        assertUndoBlocked(err, { criticArtifacts: 1 })
        return true
      },
    )
  } finally {
