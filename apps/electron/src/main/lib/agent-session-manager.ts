...
function readIndex(): AgentSessionsIndex {
  ...
  if (data) {
    ...
    if (permissionModeMigrated || thinkingDefaultMigrated || linguistRolesMigrated || retiredClaudeRuntimeMigrated || data.version < INDEX_VERSION) {
...
export function createAgentSession(
  ...
  workspaceId?: string,
  modelId?: string,
  agentCwdModeOrRuntime?: AgentCwdMode | 'pi',
  sessionWorkbenchLayoutOrLinguistBinding?: SessionWorkbenchLayout | AgentSessionLinguistBinding,
  linguistBindingOverride?: AgentSessionLinguistBinding,
): AgentSessionMeta {
  const index = readIndex()
  const now = Date.now()
  const agentCwdMode = agentCwdModeOrRuntime === 'pi' ? undefined : agentCwdModeOrRuntime
  const sessionWorkbenchLayout = typeof sessionWorkbenchLayoutOrLinguistBinding === 'string'
    ? sessionWorkbenchLayoutOrLinguistBinding
    : undefined
  const linguistBinding = typeof sessionWorkbenchLayoutOrLinguistBinding === 'object'
    ? sessionWorkbenchLayoutOrLinguistBinding
  ...
  try {
    ...
    // 若有工作区，创建 session 级工作台；计划和附件目录按需创建。
    if (workspaceId) {
      const ws = getAgentWorkspace(workspaceId)
      if (ws) getAgentSessionWorkspacePath(ws.slug, meta.id)
    }
...
async function forkPiAgentSession(
  ...
  const destinationWorkspace = newMeta.workspaceId
    ...
    : undefined
  const destinationActiveWorktree = options ? undefined : sourceActiveWorktree
  const destDir = resolveAgentCwd(
    destinationWorkspace,
    newMeta.id,
    newMeta.agentCwdMode,
    destinationActiveWorktree,
  )
  const destWorkbenchDir = resolveAgentWorkbenchDir(destinationWorkspace, newMeta.id)
...
/** Pi 分叉只接受仍存在的受管 session artifact。 */
export function hasAgentSessionNativeForkArtifact(session: AgentSessionMeta): boolean {
  return Boolean(session.sdkSessionId && session.piSessionFile && existsSync(session.piSessionFile))
}
