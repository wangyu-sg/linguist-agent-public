...
当前 worktree 的 Proma 稳定基线已推进到 `v0.19.37`，正式合并为 `4a7cbcec`；LA 发布候选版本为 `0.17.72`。审查候选 `479120b7` 已快进合入 `main`，首次远程 CI `34180940506` 在传统滚动条下发现窄窗列宽与表头错位并正确阻断发布；现已统一表头/列表的滚动条占位，本地重新打包后 Linguist 31 PASS / 0 FAIL / 2 MANUAL，等待修正提交的远程 CI 与自动发布；本机安装版由用户通过自动更新升级。
