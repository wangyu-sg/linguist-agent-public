...
- Common Quality Contract 内置于单一 Prompt Builder，四岗位 Markdown 的唯一真源位于 `resources/linguist-roles/`；岗位资源缺失时使用短 fallback，不再叠加旧 Linguist Skill。诊断会区分 Digest 完整、部分、跳过与长度裁减。
...
- `cat_export_asset` 公开参数为 `validation: verified/as-is`。两者默认不覆盖；只有显式 `overwrite=true` 才原子替换普通文件。`verified` 必须通过完整交付与结构硬规则，`as-is` 允许未完成内容但仍保留路径安全和格式 round-trip 验证。原生 UI 保持单文件 picker 与 `verified` 导出；其异步读盘、manifest 校验和安全复制已收口到共享服务。
...
- Workbook Mapping 可预览建议并持久化复用；Voice Profile 与复用 TM Unit 的 approved exemplar 可结构化写入并按需返回给 Agent。
...
3. 用真实 memoQ MQXLIFF 样本验证 detect / import / modify / export / reimport 和批注兼容。
