...
| 2026-07-26 | PB-042 | 无逐字复制——会话 CAT 工具解析、冲突检查、fake model 场景与打包探针均按本仓既有 PB-034/PB-040 接缝全新编写；为修复 Electron CJS 启动回归，将 Pi `defineTool` 的类型保持行为改为本地泛型恒等函数（仅依据已安装包公开类型/运行时行为，未复制 SDK 文件） | `apps/electron/src/main/lib/linguist/session-cat-tools.ts`、`apps/electron/scripts/smoke/probe-cat-tools.ts`、`packages/linguist-cat-tools/src/factory.ts` | AGPL-3.0 新代码 |
