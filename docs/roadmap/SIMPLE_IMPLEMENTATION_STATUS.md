...
| 版本、基线与文档 | DONE | App `0.17.60`、Shared `0.1.101`；baseline / ledger / deviations / canonical facts 已同步。 |
| 自动回归 | DONE WITH KNOWN FAILURES | typecheck 与同步专项通过；全量 `1537 pass / 7` 个既有 MCP HTTP 恢复失败，本轮无新增失败。 |
| 未打包 Electron build | DONE | main / workers / runtime / preload / renderer / CLI / Agent Island / EventKit / resources 均通过。 |
| Packaged 验证 | PENDING | 尚未运行 `smoke:pack`、packaged startup 或 vertical smoke。 |
...
- Electron Linguist `214/214`；全量 `1537 pass / 7 fail`。
