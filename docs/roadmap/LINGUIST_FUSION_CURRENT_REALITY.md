...
- v0.18.2 的统一右侧工作区、Terminal、浏览器操作、归档分组与 Workspace 稳定性修复已直接继承；冻结的 `linguistContext` 在 Renderer、IPC 与主进程即时提交路径中保持一致。
...
- About / Linguist Diagnostics 的 Proma Base 由 `linguist-build-metadata.ts` 展示，并与 `proma-baseline.json` 对账为 `v0.18.2@92a635fa` / `fc8e8f3d`。
