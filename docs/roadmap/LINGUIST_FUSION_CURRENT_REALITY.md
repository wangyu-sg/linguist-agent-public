...
| Shared | `0.1.92` |
| CAT Core / Formats / Store / Tools | `0.0.19 / 0.0.10 / 0.0.34 / 0.0.32` |
...
- Common Quality Contract 内置于单一 Prompt Builder，四份岗位 Markdown 位于 `resources/linguist-roles/` 并作为岗位提示词唯一真源；不再注入旧 project role Skill。Prompt 诊断区分岗位 fallback、Digest 完整/部分/跳过与长度裁减。
...
- 原生 UI 按当前范围保持单文件 picker 和 `verified` 导出；文件使用同一 fd 异步限额读取，manifest 校验与 secure copy 在 ProjectDelivery 单点执行。
