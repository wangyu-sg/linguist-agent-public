...
| 项目 | 当前事实 |
|---|---|
| Proma Base / formal merge | `v0.19.23@1ab22a17` / `8ff00976` |
| App / Electron | `0.17.68` / `43.2.0` |
...
- v0.19.23 的会话拖入输入框引用、单一委派观察 Tab、Vault 折叠侧栏及 Markdown / Preview / Changes 修复已直接继承；冻结的 `linguistContext` 在 Renderer、IPC 与主进程即时提交路径中保持一致。
...
- About / Linguist Diagnostics 的 Proma Base 由 `linguist-build-metadata.ts` 展示，并与 `proma-baseline.json` 对账为 `v0.19.23@1ab22a17` / `8ff00976`。

## 已验证与未验证

- typecheck、默认 CI 关键回归 `355/355`、boundary `4/4`、fusion `14/14`、Host Seam `9/9` 与历史冲突分类 `9/9` 通过。
