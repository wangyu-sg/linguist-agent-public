...
export async function readTaskMessageQueue(repoRoot: string, locator: TaskLocator): Promise<TaskMessageQueue> {
...
export async function updateTaskMessageQueue(
  repoRoot: string,
  locator: TaskLocator,
  ...
  try {
    const current = await readTaskMessageQueue(repoRoot, locator);
