 ...
 *
 * CAT packages must stay importable in both Node and browser-ish contexts, so
 * the default hasher is a tiny pure-TS implementation (no node:crypto, no
 * WebCrypto async API). Callers may inject their own `HashFn`.
 */
