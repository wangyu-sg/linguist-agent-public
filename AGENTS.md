...
- 普通代码提交不修改版本号。
- `@proma/core`、`@proma/shared`、`@proma/session-core`、`@proma/ui` 的版本跟随当前 Proma 上游。
- `@linguist/*` 只在其公开合同、Schema 或可发布行为真实变化时递增。
- `apps/electron/package.json` 只由 Release 流程递增。
- 不为测试、文档、内部重构或上游同步噪音单独提高内部包版本。
