# TODO

只列仍需完成的工作；当前实现和验证状态见 [当前事实](./CURRENT_FACTS_SIMPLE.md)。

- [ ] 排查用户报告的 macOS 红绿灯窗口按钮问题：复现条件和根因尚未确认，按用户要求留到下一轮修复。
- [ ] 在获准 Phrase 测试工作完成浏览器输入、原生结构、保存、TM/QA 与 Accepted 校准，并记录同条件计时；不操作客户批次进行开发测试。
- [ ] VALID-002：在当前 packaged app 中用真实 Provider 和 3–5 个匿名 Segment 完成 General → Translator → Reviewer → Proofreader → QA → verified export → re-import，逐段核对冻结范围、阶段决定、结构与状态。
- [ ] VALID-001：同一模型、同一 reasoning、同一真实语言任务，对比 Web Chat、旧 LA 与当前 LA。
- [ ] 使用真实 Phrase / memoQ 平台产物验证导入、修改、导出、重导、inline code、状态和审校批注。
- [ ] VALID-003：从可用构建开始累计 14 个真实日用日，记录阻断与数据完整性。
- [ ] 真机人工：IME composition、Native Open/Save、Companion round-trip、VoiceOver、keyboard-only、窄窗与拖拽 / resize。
- [ ] Workbench 视口验收：1440px、1024px 和 200% 下上下文清楚、无重叠；右侧切换/分屏保留当前会话、批次与 CAT 状态。
- [ ] 取得上述证据后再裁决个人 Alpha 资格。
